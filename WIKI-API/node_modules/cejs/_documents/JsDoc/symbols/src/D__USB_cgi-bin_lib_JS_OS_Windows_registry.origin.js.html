...
<span class='line'> 65</span> 	WSH_registry.Base	設定工作的基準，這應該是個目錄，將會附加在每個key前面
<span class='line'> 66</span> 
<span class='line'> 67</span> 	WSH_registry(key)	WshShell.RegRead()
<span class='line'> 68</span> *undo*				key假如輸入object，會將之一一分開處理，此時WSH_registry.Err會包含所有發生的錯誤，WSH_registry.Err[0]=發生錯誤的數量
<span class='line'> 69</span> *undo*	WSH_registry(key,0,'info')	完整資訊（包括type）
...
<span class='line'>133</span> 用.apply()實作prototype之function，不另外寫。
...
<span class='line'>164</span> REG_BINARY 二進位制資料。登錄檔編輯器會以十六進位的記數法來顯示二進位制的資料，而你必須用十六進位制的記數法 來輸入二進位的資料。舉個例子來說，如REG_BINARY值為0x02 0xFE 0xA9 0x38 0x92 0x38 0xAB 0xD9。
<span class='line'>165</span> REG_DWORD 雙字組值(32-bits)。很多REG_DWORD內容值都使用像是布林值(0 或1、true或false、yes或者是no)。你也可以看到時間值以百萬秒(millisecond)的方式被放在REG_DWORD當中(1000 即1秒)。32-bit未指定的範圍可以從0到4,294,967,295，並且32-bit指定數值範圍可以從-2,147,483,648到 2,147,483,647。你可以使用十進位制或者是十六進位制的方法來編輯這些數值。如REG_DWORD值可表示為0xFE020001及 0x10010001。
<span class='line'>166</span> REG_DWORD_BIG_ENDIAN 雙字組(Double-word)值以最顯著的方式被存放在記憶體當中。這些位元的順多與REG_DWORD的順序相反。舉個例子來說，數值 0x01020304被以0x01 0x02 0x03 0x04的型態放置在記憶體當中，你並不會在Intel-based 的架構中看到諸如此類的架構。
<span class='line'>167</span> REG_DWORD_LITTLE_ENDIAN 雙字組值至少有顯者的位元組被儲存在記憶體當中，這個型態跟REG_DWORD是相同的，並且因為Intel-based的架構是以這種格式來儲存數值 的，在Windows XP當中，它是最普遍的數值。舉例來說，0x01020304以0x04 0x03 0x02 0x01的內容被存放在記憶體當中，登錄檔編輯器並不提供用來建立REG_DWORD_LITTLE_ENDIAN 值的能力，因為這個數值資料型態對於REG_DWORD在登錄檔當中的角色而言是相同的。
<span class='line'>168</span> REG_EXPAND_SZ 變數長度的文字資料。以這種資料型態放置的資料可以是變數、及在使用它們之前，用來延伸這些變數的數值的程式。舉個例子來說，REG_EXPAND_SZ 值包含了%USERPROFILE%\Favorites在程式使用它之前，可能被延伸為C:\Documents and Settings\Jerry\Favorites 。這些登錄器API (Application Programming Interface)會依照所呼叫的程式來延伸環境變數REG_EXPAND_SZ字串，所以它在程式沒有擴充他們的時候，是沒有作用的。您可以看看第十章「引用使用者資訊檔」，以學習更多此類內容值的型態，以修正一些有趣的問題。
<span class='line'>169</span> REG_FULL_RESOURCE_DESCRIPTOR 資源列表會將裝置及裝置的驅動程式列示出來。這也就資料型態對於PNP裝置來講很重要的原因。登錄檔編輯器並不提供任何方去來製作這種型態的內容值，但是 它允許你顯示它們。你可以查看HKLM\HARDWARE\DESCRIPTION\Description做為這類資料型態的範例。
<span class='line'>170</span> REG_LINK 它是一個連接，而您無法建立REG_LINK值。
...
<span class='line'>173</span> REG_QWORD Quadruple-word值(64-bits)。此一型態的資料與REG_DWORD型態相似，但是它包含了 64 bits而不是32 bit。而支援此一型態的作業系統只有Windows XP 64-Bit Edition。你可以使用十進位或者是十六進位的記數方法來查看及編輯此類的登錄值。 0xFE02000110010001為REG_QWORD的一個例子。
...
<span class='line'>175</span> REG_QWORD_LITTLE_ENDIAN 至少有Quadruple-word值儲存在記憶體當中。這種型態與REG_QWORD相同。您可以查看REG_DWORD_LITTLE_ENDIAN 取得更多的資訊。登錄檔編輯器並不提供製作REG_QWORD_LITTLE_ENDIAN 內容的能力，因為這個值的型態對於登錄檔中的REG_QWORD而言是唯一的。
...
<span class='line'>227</span> </span><span class="COMM">//	分開base與path，並作檢查。</span><span class="WHIT">
...
<span class='line'>250</span> </span><span class="WHIT">  </span><span class="COMM">//	輸入 root 之 ValueName，如 HKEY_CURRENT_USER\value</span><span class="WHIT">
...
<span class='line'>375</span> </span><span class="COMM">//	一次性功能，不通過創建object</span><span class="WHIT">
<span class='line'>376</span> </span><span class="COMM">/*	ӥ۞某 path: Subkey(機碼) 之 {ValueName:(int)ValueType} 資訊。無 Value 會 return undefined
<span class='line'>377</span> registry_function.getValue('HKEY_CLASSES_ROOT\\.odp')	傳ީ設值
...
<span class='line'>385</span> </span><span class="WHIT"> </span><span class="KEYW">if</span><span class="PUNC">(</span><span class="PUNC">!</span><span class="NAME">oOutParam</span><span class="WHIT"> </span><span class="PUNC">||</span><span class="WHIT"> </span><span class="NAME">oOutParam.sNames</span><span class="PUNC">==</span><span class="KEYW">null</span><span class="PUNC">)</span><span class="KEYW">return</span><span class="PUNC">;</span><span class="WHIT">	</span><span class="COMM">//	error 大概都是 ==null，可能因為輸入value而非key值</span><span class="WHIT">
...
<span class='line'>411</span> </span><span class="COMM">/*	一次性功能，不通過創建object
...
<span class='line'>423</span> </span><span class="WHIT"> </span><span class="KEYW">if</span><span class="PUNC">(</span><span class="NAME">aNames</span><span class="PUNC">!=</span><span class="KEYW">null</span><span class="PUNC">)</span><span class="PUNC">{</span><span class="WHIT">	</span><span class="COMM">//	error 大概都是 ==null，可能因為: 1.無Subkey 2.輸入value而非key值</span><span class="WHIT">
...
<span class='line'>487</span> </span><span class="WHIT"> </span><span class="COMM">//	若可create(並access)，表示不存在（需刪掉建出來的），return false。否則unknown，return undefined。</span><span class="WHIT">
...
<span class='line'>594</span> </span><span class="COMM">//	設為true記得setValue後馬上改回來，否則可能出現自動加subkey的情形。</span><span class="WHIT">
