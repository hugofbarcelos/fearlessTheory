...
<span class='line'> 16</span>  * 本檔案包含了呼叫其他 library 需要用到的 function，以及常用 base functions。&lt;br/>
...
<span class='line'> 79</span> 本 library 大量使用了 arguments.callee，但這與 ECMAScript design principles 不甚相符，需要避免。
...
<span class='line'>176</span>  * 在舊的 browser 中，undefined 可能不存在。
...
<span class='line'>333</span>  * 若在 function 中 eval 以獲得 local variable，在舊 browser 中須加 var。
...
<span class='line'>346</span> 		use window.execScript(code,"JavaScript") in IE: window.execScript() 將直接使用全局上下文環境，因此，execScript(Str)中的字符串Str可以影響全局變量。——也包括聲明全局變量、函數以及對象構造器。
...
<span class='line'>434</span> </span><span class="WHIT">	</span><span class="COMM">//	在 IE8, IE9 中，get_object_type.call(WScript) 為 '[object Object]' !!</span><span class="WHIT">
...
<span class='line'>451</span> </span><span class="WHIT">	</span><span class="COMM">//	在 IE8, IE9 中，get_object_type.call(WScript) 為 '[object Object]' !!</span><span class="WHIT">
...
<span class='line'>514</span>  * &lt;a href="http://lifesinger.org/blog/2009/02/javascript-type-check-2/" accessdate="2009/12/6 19:10">JavaScript类型检测小结（下） - 岁月如歌&lt;/a>&lt;br/>
...
<span class='line'>533</span> </span><span class="WHIT">					</span><span class="COMM">// 若 value 為 function 且無 Class，或為 object 時，測試其 constructor 之 Class。</span><span class="WHIT">
...
<span class='line'>661</span> </span><span class="WHIT">		</span><span class="COMM">//	須注意，在 firefox 3 中，typeof [object HTMLObjectElement] 之外的 HTMLElement 皆 === 'function'，</span><span class="WHIT">
...
<span class='line'>674</span>  * （雖然不喜歡另開 name-space，但以 2009 當下的 JsDoc Toolkit 來說，似乎沒辦法創造 enumeration。）
...
<span class='line'>784</span> 		 * 存放在 registry 中的 path，通常指的是 library 在 File System 中的 base path
...
<span class='line'>1125</span> 	 * 在 WScript 中 需要用 _.log，其他則可用 log。
...
<span class='line'>1314</span>  * 惰性求值（Lazy Evaluation），又稱懶惰求值、懶漢求值
...
<span class='line'>1376</span>  * 依序載入 resource，用於 include JavaScript 檔之類需求時，取得檔案內容之輕量級函數。&lt;br/>
<span class='line'>1377</span>  * 除 Ajax，本函數亦可用在 CScript 執行中。
...
<span class='line'>1436</span> </span><span class="WHIT">		</span><span class="COMM">//	Opera 11.50: 不會 throw，但是 .responseText === ''。</span><span class="WHIT">
...
<span class='line'>1463</span> </span><span class="WHIT">	</span><span class="COMM">//	workaround for Opera: Opera 11.50: 不會 throw，但是 .responseText === ''。</span><span class="WHIT">
...
<span class='line'>1467</span> </span><span class="WHIT">	</span><span class="COMM">//	當在 local 時，成功的話 status === 0。失敗的話，除 IE 外，status 亦總是 0。</span><span class="WHIT">
...
<span class='line'>1488</span>  * enablePrivilege 似乎只能在執行的 function 本身或 caller 呼叫才有效果，跳出函數即無效，不能 cache，因此提供 callback。
...
<span class='line'>1549</span> </span><span class="WHIT">		</span><span class="COMM">//	Mozilla的安全機制是透過PrivilegeManager來管理，透過PrivilegeManager的enablePrivilege()函式來開啟這項設定。</span><span class="WHIT">
...
<span class='line'>1615</span>  * //	引數為本.js檔名。若是更改.js檔名，亦需要同步更動此值！
...
<span class='line'>1889</span> </span><span class="WHIT">			</span><span class="COMM">//	若是在 .setup_module 中的話，可以探測得到 name_space？（忘了）</span><span class="WHIT">
...
<span class='line'>1958</span>  * 不使用 eval 的方法，get the module namespace of specific module name.
...
<span class='line'>2018</span>  *          something error, e.g., 未成功 load，code_for_including
...
<span class='line'>2125</span> </span><span class="WHIT">				</span><span class="COMM">//	若有未載入之 dependencies，則不載入 module。</span><span class="WHIT">
...
<span class='line'>2153</span> </span><span class="WHIT">	</span><span class="COMM">// else	所有需求皆已在 queue 中，因此最後總**有機會（不包括發生錯誤的情況！）**會被 load，故 skip。</span><span class="WHIT">
...
<span class='line'>2190</span> </span><span class="WHIT">		</span><span class="COMM">// 保留原先的 name-space，for 重新定義</span><span class="WHIT">
...
<span class='line'>2530</span>  * @param {String} [module_name]	呼叫的 module name。未提供則設成 library base path，此時 file_name 為相對於 library 本身路徑的檔案。
...
<span class='line'>2552</span>  * 無法以 Ajax 載入時，若未設定 callback，會回傳錯誤。若設定 callback，會以同步(synchronous)的方式載入 module，於載入完成執行 callback。
<span class='line'>2553</span>  * 		若因為 browser 安全性設定而無法取得則會回傳 -1，表示將以同步的方式載入 module。因為 module 尚未載入，在此階段尚無法判別此 module 所需之 dependency list。此 list 會被作為引數傳入 callback。
<span class='line'>2554</span>  * 
<span class='line'>2555</span>  * 注意：以下的 code 中，CeL.warn 不一定會被執行（可能會、可能不會），因為執行時 log 可能尚未被 include。&lt;br/>
...
<span class='line'>2645</span> </span><span class="WHIT">			</span><span class="COMM">//	不能直接用 .get_file()，得採用其他方法的狀況。但只在有 callback 時才 include，否則當下 block 的都沒執行，可能出亂子。</span><span class="WHIT">
...
<span class='line'>2816</span> </span><span class="WHIT">						</span><span class="COMM">//	error on IE5–7: Error: Object doesn't support this action</span><span class="WHIT">
...
<span class='line'>2822</span> </span><span class="WHIT">						</span><span class="COMM">//	error on IE5–7: Error: Object doesn't support this action</span><span class="WHIT">
...
<span class='line'>2893</span> </span><span class="WHIT">	</span><span class="COMM">//	若是到這邊還沒 load，會造成問題。</span><span class="WHIT">
...
<span class='line'>2913</span> </span><span class="WHIT">		</span><span class="COMM">//	不是 module，僅僅為指定 function 的話，直接等一下再看看。</span><span class="WHIT">
...
<span class='line'>3028</span> 	//	可能闕如: 臨時新增用，是為了預防有 call C，但 dependency 為 A→B→C 的情況。重複使用 queue 但不檢查 require 可能造成 B 與 C 被放在同一 synchronous_group。
...
<span class='line'>3093</span>  *            sequence of set_run.arguments. 不修改 work_queue===set_run.arguments，直接以 work_queue_index 為開始值。
...
<span class='line'>3187</span> 如果 check 發現 _path_ dependencies 尚未 load，則把 dependencies 加入 to_load_path|to_load_module，去除 (to_load_path|to_load_module)[_path_]，
...
<span class='line'>3230</span> </span><span class="WHIT">						</span><span class="COMM">//	假如是同一批的 M1, M2 都需要 P0，則跑到 M2 時 P0 不需要設定第二次，但需要把 M2 移到下一批次。</span><span class="WHIT">
...
<span class='line'>3239</span> </span><span class="WHIT">						</span><span class="COMM">//	因為是指向 Object，因此不需要再設定 synchronous_group.to_load_path。為防節外生枝，直接改 .path_count，不 cache。</span><span class="WHIT">
...
<span class='line'>3260</span> </span><span class="WHIT">						</span><span class="COMM">// 假如是同一批的 M1, M2 都需要 M0，則跑到 M2 時 M0 不需要設定第二次，但需要把 M2 移到下一批次。</span><span class="WHIT">
...
<span class='line'>3269</span> </span><span class="WHIT">						</span><span class="COMM">//	因為是指向 Object，因此不需要再設定 synchronous_group.to_load_module。為防節外生枝，直接改 .module_count，不 cache。</span><span class="WHIT">
...
<span class='line'>3343</span> 	 * 有未載入之 dependencies，僅能從 callback 傳入此 module 所需之 dependency list 來處置。
<span class='line'>3344</span> 	 * TODO: 確認若是load錯誤時，會不會跳過 check_loading 不執行。
...
<span class='line'>3350</span> </span><span class="WHIT">	</span><span class="COMM">//	synchronous_group.to_run: 若要處理 .to_run 的 require，則 check_loading() 還是需要設定。</span><span class="WHIT">
...
<span class='line'>3366</span> </span><span class="WHIT">					</span><span class="COMM">//	若有不存在的 module，因為會以 .include_resource 載入，在 MSIE 中會 throw。</span><span class="WHIT">
...
<span class='line'>3389</span> </span><span class="WHIT">			</span><span class="COMM">//_.debug('Move to next synchronous load group. 本同步載入組已全部載入，' + (synchronous_group ? '進入下一同步載入組。' : work_queue_index &lt; work_queue.length ? '繼續下一組指定的工作 [' + work_queue_index + '/' + work_queue.length + ']。' : '本次指定的 ' + work_queue_index + ' 項工作已全部執行完成。') + (timeout > 4 ? 'timeout ' + timeout + ' 超過 4ms，設定 timeout。' : ''), 1, 'check_run.check_loading');</span><span class="WHIT">
...
<span class='line'>3446</span> </span><span class="WHIT">		</span><span class="COMM">//_.debug('把能處理的 function 先處理完了，刪除synchronous_group.to_run 的資料。', 2);</span><span class="WHIT">
...
<span class='line'>3521</span>  *            CeL.env.module_name_separator ('.') 結尾。若輸入 String，則以 separator 或 '|' 分割。
...
<span class='line'>3593</span> </span><span class="WHIT">	</span><span class="COMM">//	解析 dependency_list，將所須 functions/modules 置於 variable_hash/module_hash 中。</span><span class="WHIT">
...
<span class='line'>3609</span> </span><span class="WHIT">			</span><span class="COMM">//	不用 _.to_module_name，因為會加油添醋。</span><span class="WHIT">
...
<span class='line'>3677</span>  * //	若無法 load CeL.data，將會 throw
...
<span class='line'>3685</span>  * //	若無法 load CeL.data，將會 throw
...
<span class='line'>3691</span>  * //	若無法 load CeL.data，將會 throw
...
<span class='line'>3697</span>  * @param	{Array|String} dependency_list	list of dependency function/module/variable required. module 須以 '.' 結尾。若輸入 String，則以 ',' 分割。
...
<span class='line'>3787</span> 	 * 理論上 '.'.split(/\./).length 應該是 2，但 IE 5–8 中卻為 0!
...
<span class='line'>3924</span> 		.Items().Item(), colVerbs = objFolderItem.Verbs(); // 假如出意外，objFolder==null
...
<span class='line'>3947</span> //測試可寫入的字元:0-128,最好用1-127，因為許多編輯器會將\0轉成' '，\128又不確定
