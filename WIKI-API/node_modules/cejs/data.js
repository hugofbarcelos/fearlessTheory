...
function module_code(library_namespace) {
	...
	function Pair(source, options) {
		if (!is_Pair(this)) {
			library_namespace
					.warn('Pair: Please use (pair = new Pair()) instead of (pair = Pair())!');
	...
	library_namespace.set_method(Pair.prototype, {
			...
			if (library_namespace.is_Object(source)) {
				...
				if (typeof source === 'string') {
					...
					if (separator)
						...
						//		
						'Pair.add: Can not determine the field separator!');
						source = [ source ];
					}
				}
				...
				if (Array.isArray(source)) {
					...
					item_processor
					//
					= typeof options.item_processor === 'function'
					//
					? options.item_processor : null;
					...
					if (!separator && typeof source[0] === 'string') {
						...
						for (var i = 0; i < length; i++)
							if (typeof source[i] === 'string'
								...
								//
								+ new RegExp(separator), 3, 'Pair.add');
								break;
							}
						if (!separator)
							throw new Error(
							//
							'Pair.add: No assignment sign detected! 請手動指定！');
					}

					library_namespace.debug('Add '
					//
		...
		save : function(path, encoding, save_new) {
			if (!library_namespace.write_file)
				throw new Error(
						'Please include application.OS.Windows.file first!');

			if (path !== this.path)
				path = this.path;
			else if (!save_new && this.remove_comments)
				library_namespace
						.warn('移除注解後再存檔，會失去原先的注解！請考慮設定 save_new flag。');
			...
			if (data.length > 0) {
				library_namespace.debug(
						[ save_new ? 'Appending ' : 'Writing ', data.length,
								' data to (' + encoding, ') [', path, ']' ], 2,
						'Pair.save');
				...
				if ((key = this_pair[value]) || key === '') {
					if (key in pair) {
						if (pair[key] !== is_number ? +value : value)
							library_namespace
									.warn('Pair.reverse: duplicated key ['
											+ key + ']');
					} else {
